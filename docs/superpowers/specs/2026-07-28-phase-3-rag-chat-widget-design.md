...
- `public/widget.js` (plain JS, no framework, reads `data-org` off its own `<script>` tag via `document.currentScript`) creates an iframe pointed at `/widget?org=...&host=<encodeURIComponent(location.origin)>` on the app's own domain.
- Because the iframe is same-origin with the API, the browser's `Origin` header on its fetches is the app's own domain, not the embedding site's — so the widget page forwards the `host` query param explicitly as `widgetOrigin` in the chat request body, and that's what gets checked against `allowedDomains`. This also means **no CORS handling is needed** for `/api/chat` — the actual fetch is same-origin by construction; only the iframe embed itself is cross-origin, which needs no special headers (the app doesn't currently set any `X-Frame-Options`/CSP that would block being framed — confirmed, `next.config.ts` is a bare scaffold).
- Widget page is a normal React route (`src/app/widget/page.tsx`), themed from `Organization.widgetConfig` (`primaryColor`/`greeting`/`position` — confirmed these exist and are set at org creation).
...
- `useChat` + `DefaultChatTransport`'s `prepareSendMessagesRequest` option — confirmed this fully **replaces** (not merges) the outgoing request body, so the widget can POST exactly `{siteKey, widgetOrigin, conversationId, message}` instead of the SDK's default `{id, messages[], trigger, messageId}` shape, while still getting `useChat`'s SSE parsing and reconnect handling. No custom `ChatTransport` implementation needed.
...
- `/api/chat/route.ts` needs no `runtime` export (defaults to `nodejs`, confirmed compatible with the MongoDB driver, matching the existing `ai-smoke` route). Next.js 16 has no breaking changes affecting this.
